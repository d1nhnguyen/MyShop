...
async function seed() {
    try {
        ...
        const hashedPassword = await auth_1.AuthUtils.hashPassword('Password@123');
        const users = await Promise.all([
            prisma.user.create({
                data: { username: 'admin', email: 'admin@myshop.com', password: await auth_1.AuthUtils.hashPassword('Admin@123456'), role: 'ADMIN' },
            }),
            prisma.user.create({
        ...
        for (const d of discountData) {
            const disc = await prisma.discount.create({
                data: {
                    ...
                    type: d.type,
                    value: new client_1.Prisma.Decimal(d.value),
            ...
            for (const item of group.items) {
                ...
                const product = await prisma.product.create({
                    data: {
                        ...
                        costPrice: new client_1.Prisma.Decimal(item.cost),
        ...
        if (allCreatedProducts.length > 0 && customers.length > 0) {
            ...
            for (let i = 1; i <= numberOfOrdersToSeed; i++) {
                ...
                const createdOrder = await prisma.order.create({
                    data: {
                        orderNumber: orderNum, customerId: getRandomCustomer().id, userId: selectedUser.id, status: status,
                        subtotal: new client_1.Prisma.Decimal(subtotal), discountAmount: new client_1.Prisma.Decimal(0), taxAmount: new client_1.Prisma.Decimal(0),
                        total: new client_1.Prisma.Decimal(subtotal), createdAt: orderDate,
                        completedAt: status === 'COMPLETED' ? orderDate : null,
                        orderItems: {
                            create: items.map(i => ({
                                productId: i.productId, quantity: i.quantity, unitPrice: i.unitPrice,
                                subtotal: new client_1.Prisma.Decimal(Number(i.unitPrice) * i.quantity),
                                discountAmount: new client_1.Prisma.Decimal(0),
                                total: new client_1.Prisma.Decimal(Number(i.unitPrice) * i.quantity),
                            }))
                        }
                    }
                });
                if (status === 'COMPLETED') {
                    ...
                    await prisma.commission.create({
                        data: {
                            ...
                            orderTotal: new client_1.Prisma.Decimal(subtotal),
                            commissionRate: new client_1.Prisma.Decimal(COMMISSION_RATE),
            ...
            for (const customer of customers) {
                ...
                await prisma.customer.update({
                    ...
                    data: { totalSpent: new client_1.Prisma.Decimal(totalSpent) },
                });
            }
